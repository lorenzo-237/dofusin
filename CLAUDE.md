...
**Auth is Discord OAuth2 only — no password is ever stored.** `LoginForm` is a single "Se connecter avec Discord" button calling `useAuth().loginWithDiscord()`; there's no separate register step (a fresh Discord account just lands in `/onboarding` instead, see above). The full flow — deep link `dofusin://auth/callback`, `state` CSRF check, the `isTauri() && VITE_API_STRATEGY === "http"` gate that skips the whole browser round-trip in favor of a mock session everywhere else (plain browser tab, or mock strategy even inside Tauri) — is documented in `docs/discord-auth-deep-link.md`. `AuthProvider` owns the `onOpenUrl` listener (registered once) and the pending-login promise (a ref resolved/rejected by that listener) that `loginWithDiscord()`/`cancelDiscordLogin()` coordinate through.
